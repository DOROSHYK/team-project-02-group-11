...
export default class ServerAPI {
  ...
  getPopularFilmList() {
    return fetch(
      `${this.baseURL}trending/movie/week?api_key=${this.APIkey}&page=${this.page}&language=${this.language}&append_to_response=videos,images`,
    )
      .then(response => {
  ...
  getFilmByKeyword(keyWord) {
    ...
    return fetch(
      `${this.baseURL}search/${this.mediaType}?api_key=${this.APIkey}&query=${keyWord}&page=${this.page}&language=${this.language}`,
    )
      .then(response => {
        if (response.ok) return response.json();
        throw new Error('Error fetching data');
  ...
  getFilmInfoById(id) {
    return fetch(
      `${this.baseURL}${this.mediaType}/${id}?api_key=${this.APIkey}&language=${this.language}`,
    )
      .then(response => {
        if (response.ok) return response.json();
        throw new Error('Error fetching data');
  ...
  getGenresList() {
    return fetch(
      `${this.baseURL}genre/${this.mediaType}/list?api_key=${this.APIkey}&language=${this.language}`,
    )
      .then(res => res.json())
  ...
  getObjectForRender(filmData) {
    return {
      ...
      genres: filmData.genres
        ...
        : this.getGenreById(filmData.genre_ids),
      year:
        filmData.release_date === undefined
  ...
  getTrailerFetchId() {
    return fetch(
      `http://api.themoviedb.org/3/movie/157336/videos?api_key=fd6eb8c46328921cf1133ef8e877d946`,
    )
      .then(response => {
        if (response.ok) return response.json();
        throw new Error('Error fetching data');
